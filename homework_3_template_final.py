...
def fMSE (w, Xtilde, y):
    fse = 1/5000 * (y - Xtilde.T.dot(w)).T.dot(y - Xtilde.T.dot(w))
    # exp(z_k/for every k to c(exp(z_k)))
    return fse
...
def gradientDescent (Xtilde, y, alpha = 0.):
    ...
    for i in range(T):
        gradient = Xtilde.dot(1/(np.exp(Xtilde.T).dot(np.exp(w))-y).dot(np.exp(Xtilde.T).dot(np.exp(w))).dot(1-np.exp
        (Xtilde.T).dot(np.exp(w))))
